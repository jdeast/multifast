...
;
;-
pro multifast_omc, filename, period, t0=t0, ps=ps, epsname=epsname, log=log
...
if keyword_set(ps) or n_elements(epsname) ne 0 then begin
    if n_elements(epsname) eq 0 then epsname = 'omc.ps'
    mydevice = !d.name
    set_plot, 'PS'
    !p.font=0
    aspect_ratio=1.5
...
xmin = min(epoch) & xmax = max(epoch)
xdiff = xmax-xmin
xmin = xmin - xdiff*0.33
xmax = xmax + xdiff*0.05
plot, fix(epoch), omc,psym=3, yrange=[ymin,ymax],xrange=[xmin,xmax],$
  xtitle='Epoch',ytitle='O-C (min)';,xticks=4
...
printf, log, 'T0 = '+string(coeffs[0],format='(f14.6)')+' +/- '+strtrim(sigma[0],2)
print, 'T0 = '+string(coeffs[0],format='(f14.6)')+' +/- '+strtrim(sigma[0],2)
printf, log, 'P  = '+strtrim(coeffs[1],2) + ' +/- ' + strtrim(sigma[1],2)
print, 'P  = '+strtrim(coeffs[1],2) + ' +/- ' + strtrim(sigma[1],2)
printf, log, ' '
print, ' '
printf, log, 'TTV chi^2 = '+strtrim(total(((omc)/err)^2),2)
print, 'TTV chi^2 = '+strtrim(total(((omc)/err)^2),2)
printf, log, 'TTV dof = '+strtrim((n_elements(time)-2),2)
print, 'TTV dof = '+strtrim((n_elements(time)-2),2)
printf, log, 'TTV chi^2/dof = ' + strtrim(total(((omc)/err)^2)/(n_elements(time)-2),2)
print, 'TTV chi^2/dof = ' + strtrim(total(((omc)/err)^2)/(n_elements(time)-2),2)
printf, log, ''
print, ''

;; print the epochs, times, errors, etc.
printf, log, 'Epoch      T_C       ERR     O-C  (O-C)/err'
print, 'Epoch      T_C       ERR     O-C  (O-C)/err'
for i = 0L, N_elements(epoch)-1 do printf, log, epoch[i], time[i], err[i]*60, omc[i]*60d0, omc[i]/err[i], $
  format='(i4,x,f14.6,x,i4,x,f8.2,x,f6.2)'
