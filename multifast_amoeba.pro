; $Id: //depot/idl/IDL_64/idldir/lib/amoeba.pro#1 $
; Copyright (c) 1996-2007, ITT Visual Information Solutions. All
;       rights reserved. Unauthorized reproduction is prohibited.


Function multifast_amotry, p, y, psum, func, ihi, fac
...
Function multifast_Amoeba, ftol, FUNCTION_NAME=func, FUNCTION_VALUE=y, $
	NCALLS = ncalls, NMAX = nmax, P0 = p0, SCALE=scale, SIMPLEX=p, log=log
...
s = size(p)
if s[0] ne 2 then printf, log, 'Either (SCALE,P0) or SIMPLEX must be initialized'
if s[0] ne 2 then print, 'Either (SCALE,P0) or SIMPLEX must be initialized'
...
while ncalls le nmax do begin		;Each iteration
  ...
  ncalls = ncalls + 2
  ytry = multifast_amotry(p, y, psum, func, ihi, -1.0)
  if ytry le y[ilo] then ytry = multifast_amotry(p,y,psum, func, ihi, 2.0) $
  else if ytry ge y[inhi] then begin
    ysave = y[ihi]
    ytry = multifast_amotry(p,y,psum,func, ihi, 0.5)
