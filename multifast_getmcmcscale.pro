...
function multifast_getmcmcscale, bestpars, chi2func, tofit=tofit, $
                               seedscale=seedscale, bestchi2=bestchi2,$
                               angular=angular0, debug=debug, log=log
    ...
    for j=0,1 do begin
        ...
        repeat begin
            ...
            if ~finite(bestpars[tofit[i]] + mcmcscale[i,j]) or $
               ...
               printf, log, "EXOFAST_GETMCMCSCALE: Parameter "+strtrim(tofit[i],2) + $
                      " is unconstrained. Check your starting conditions"               
               print, "EXOFAST_GETMCMCSCALE: Parameter "+strtrim(tofit[i],2) + $
                      " is unconstrained. Check your starting conditions"
            endif            
            ...
            if (chi2 - bestchi2) ge 1.d0 then begin
                ...
                if keyword_set(debug) then begin
                    printf, log, 'WARNING: better chi2 found by varying parameter '+$
            ...
            if abs(minstep - maxstep) lt 1d-14 or niter gt maxiter then begin
                if not chi2changed then begin
                    if abs(bestdeltachi2 - 1.d0) lt 0.75 then begin
                        ...
                        if j eq 0 then hilow = '(hi)' $
                        else hilow = '(lo)'        
                        printf, log, 'Convergence Error: cannot find the value' + $
                          ' for which deltachi^2 = 1, parameter = ' + strtrim(tofit[i],2) + ' ' + hilow                                      
                        print, 'Convergence Error: cannot find the value' + $
                          ' for which deltachi^2 = 1, parameter = ' + strtrim(tofit[i],2) + ' ' + hilow
            ...
            if abs(chi2 - bestchi2 - 1.d0) eq 1.d0 and niter gt maxiter then begin
              printf, log, 'ERROR: changing parameter ' + strtrim(tofit[i],2) + $
            ...
            if (where(angular eq i))[0] ne -1 and $; if (where(angular eq tofit[i]))(0) ne -1 and $
              minstep gt (2.d0*!dpi) then begin 
                printf, log, 'WARNING: no constraint on angle, setting MCMC scale = PI'
                message, 'WARNING: no constraint on angle, setting MCMC scale = PI',/continue 
                mcmcscale[i,j] =  !dpi
                chi2 = bestchi2 + 1.d0
            endif
            ...
            if keyword_set(debug) then begin
                ...
                printf, log, tofit[i], str, minstep, maxstep, $
...
;if ~finite(mcmcscale[1,0]) and ~finite(mcmcscale[1,1]) then print, 'MCMC scale for PERIOD not finite'
;mcmcscale[1,0] = 9.4021845e-008
;mcmcscale[1,1] = 9.4021845e-008
...
;print, 'mcmcscales = ', total(mcmcscale,2)/2.d0
